...
class Safety():
	...
	self.CAUTION_DISTANCE = .5 #max dist to object to cause robot to slowdown
        self.SPEED = .5
	self.ANGLE = 0
	...
	# distance_ranges = 
	distances_with_angles = map(lambda (i,x): (i*msg.angle_increment - math.radians(self.LOOKING_RANGE), x), enumerate(distance_ranges))
	...
	if closest_distance < self.DANGER_DISTANCE:
	    ...
	    swerve_msg.drive.speed = self.SPEED
	    steering_constant = 1.0 #update this in testing!!!
	    swerve = steering_constant * -1 * np.sign(closest_angle) * ((math.pi/3)-abs(closest_angle))/(math.pi/3)
	    swerve_msg.drive.steering_angle = self.ANGLE + swerve
