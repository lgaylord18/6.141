...
class ConeNavigator():
	...
	self.pass_distance = float(rospy.get_param("~pass_distance"))

        rospy.loginfo("PurePuruit node initialized")
        # Init subscribers and publishers
        self.sub = rospy.Subscriber("/cone_info", cone,\
	...
	#declare variables
	self.steering = []
	self.path_distance = 1.0 #the distance we would like to pass the cone by
    ...
    def main(self):
	while True:
	    while (time.time()-self.last_time) < self.holdtime and self.got_msg:
		new_msg = AckermannDriveStamped()
		new_msg.drive.steering_angle = self.angle
	...
	angle_diff = math.atan(path_distance/L_1) #angle added to account for passing around the cone
	angle += angle_diff
