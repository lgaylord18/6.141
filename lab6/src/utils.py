...
def particle_to_pose_no_orientation(particle):
    pose = PoseStamped()
    pose.pose.position.x = particle[0]
    pose.pose.position.y = particle[1]
    pose.pose.orientation = angle_to_quaternion(0)
    pose.header = make_header('/map', stamp=None)
    return pose

def particles_to_poses_no_orientation(particles):
    return map(particle_to_pose_no_orientation, particles)